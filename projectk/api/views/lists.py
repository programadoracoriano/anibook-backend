        ...
        if request.data['status'] == 1:
            ...
            if n_episodes is None or n_episodes == '':
                ...
                if n_episodes == eps or n_episodes > eps:
                    getStatusCompleted = Status.objects.get(val=2)
                    query = AnimeStatus.objects.create(user=request.user, anime=getAnime,
                                                       status=getStatusCompleted, score=request.data['score'],
                                                       completed=1,
                                                       date=datenow)
                    msg = {"msg": "Anime moved to completed section."}
                else:
...
def CustomListAPI(request):
    if request.method == 'POST':
        ...
        if len(title) < 0 or len(title) > 100:
            ...
            msg = {"msg": "Image is Empty!"}
        else:
            data = ContentFile(base64.b64decode(image_str), name='custom_.' + ext)
            qs = CustomList.objects.create(user=request.user, title=title, image=data)
...
def AnimeCustomListAPI(request):
    ...
    if request.method == 'GET':
        ...
        checkUserExists = Profile.objects.filter(user=request.user).count()
        if checkUserExists > 0:
            ...
            qs = AnimeCustomList.objects.filter(custom_list__id=customId).order_by("anime__name").exclude(
                custom_list__user__id__in=getProfile.blockuser.values_list('id', flat=True)).exclude(
                custom_list__user__id__in=ifBlocked.values_list('user', flat=True)
            )
        elif checkUserExists == 0:
    ...
    if request.method == 'GET':
        ...
        if checkUserExists > 0:
            ...
            if status == '0':
                ...
                qs = CustomList.objects.filter(user__id=user).exclude(
